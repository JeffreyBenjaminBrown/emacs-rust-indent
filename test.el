...
;; 5:           . filter...
;; 6:           . cloned...
;; 7:           visible },  <- result expr, should be 10 (brace-col)
;; 8:        other_stuff ))
...
(test-indent test-brace-let-code 7 10 "visible (result expression)")
...
;; 2: fn example() {
;; 3:   triples.extend(
;; 4:     { let node_triples...  <- { at col 4
;; 5-8: ...
;; 9:       node_triples }  <- result expr should be 4 (brace-col)

(message "")
(message "=== Issue 5: Result expression alignment ===")
(test-indent test-result-expr-code 9 4 "node_triples (result aligns with opening {)")
